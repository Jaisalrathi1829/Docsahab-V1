...
import { EmergencyRequest, formatAge } from "./IncomingQueue";
    ...
    <div className="flex h-full flex-col bg-slate-50/50">
      ...
      <div className="bg-white px-6 py-4 border-b border-slate-200">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <div className="text-slate-500 mt-0.5" style={{ fontSize: 13 }}>
              Ambulance #{req.ambulanceId} · Request received {formatAge(req.ageSec)} ago
            </div>
          </div>
        ...
        <div className="grid grid-cols-4 gap-3 mt-5">
          ...
          <StatTile icon={<User className="h-4 w-4" />} label="Patient" value={patient.age ? `${patient.age}y · ${patient.sex}` : patient.sex} accent="slate" />
      ...
      <div className="flex-1 overflow-auto p-6 space-y-5">
        ...
        {/* Required capabilities — what the ranking engine matched this hospital on */}
          ...
          <div className="p-5 flex flex-wrap gap-2">
            {requiredServices && requiredServices.length > 0 ? (
              ...
              <span className="text-slate-400" style={{ fontSize: 13 }}>
                No specific capability requirement — general emergency care
              </span>
            )}
          </div>
        </Card>
...
/** Renders the emergency's real audit trail, newest step highlighted. */
function Timeline({ events: source }: { events: TimelineEntry[] }) {
  if (source.length === 0) {
    return (
  ...
  const events = source.map((e, i) => ({
    ...
    alert: e.status === "SEVERITY_SELECTED",
  }));
