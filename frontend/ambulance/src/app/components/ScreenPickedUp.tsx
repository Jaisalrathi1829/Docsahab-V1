...
export function ScreenPickedUp({
  ...
  busy?: boolean;
  onSelectSeverity?: (severity: AmbulanceSeverity) => void;
  onNotifyHospital?: () => void;
}) {
  // Severity reflects what the backend has actually recorded, so a failed
  // write can never leave the UI showing a triage level that wasn't saved.
  const severity = severityToAmbulanceUi(emergency?.severity ?? null);
  const notified = ["HOSPITAL_NOTIFIED", "EN_ROUTE_TO_HOSPITAL", "ARRIVED"].includes(
    emergency?.status ?? ""
  );
  ...
  const hospitalName =
    ...
    emergency?.hospitalCandidates?.find(
      (c) => c.hospitalId === emergency?.assignedHospitalId
    )?.hospitalName ??
    null;
  const hospitalReady = Boolean(emergency?.assignedHospitalId);
  const severityChosen = severity !== null;

  const select = (value: AmbulanceSeverity) => () => onSelectSeverity?.(value);
    ...
    <div className="flex flex-col h-full px-4 pb-4">
      ...
      <div className="rounded-2xl border border-[#E4E9F2] bg-white p-2 mb-3 flex items-center gap-1">
        ...
        <WorkflowStep
          index={2}
          label="Severity"
          state={severityChosen ? "done" : "active"}
        />
        <div className="h-px flex-shrink-0 w-2 bg-[#E4E9F2]" />
        <WorkflowStep
          index={3}
          label="Notify"
          state={notified ? "done" : severityChosen ? "active" : "pending"}
        />
      </div>
      ...
      <div className="rounded-2xl bg-white border border-[#E4E9F2] p-3 mb-3">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <div style={{ fontSize: 14, color: "#0B2545", fontWeight: 700, lineHeight: 1.1 }}>
                {patientAge != null ? `${patientName}, ${patientAge}` : patientName}
              </div>
            </div>
          </div>
        ...
        <div
          className="mt-2.5 flex items-center gap-2 px-2.5 py-1.5 rounded-lg"
          style={{ background: "#FFF4E0", border: "1px solid #F2C778" }}
        >
          <AlertTriangle className="w-4 h-4 text-[#E89B2B]" strokeWidth={2.6} />
          <span style={{ fontSize: 11, fontWeight: 800, color: "#8A5A12", letterSpacing: 0.3 }}>
            CRITICAL ALERT:
          </span>
          <span style={{ fontSize: 12, fontWeight: 700, color: "#4A2E07" }}>
      ...
      <div className="mb-3">
        ...
        <div className="grid grid-cols-3 gap-2">
          ...
          <button
            onClick={select("red")}
            disabled={busy}
            className="rounded-2xl p-3 text-left relative overflow-hidden transition-all duration-150 disabled:cursor-not-allowed"
            style={
              severity === "red"
                ? {
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-1.5">
              ...
              <span style={{ fontSize: 18, fontWeight: 800, color: "#066547", letterSpacing: -0.2, lineHeight: 1 }}>
                {hospitalReady ? "HOSPITAL READY" : "FINDING HOSPITAL"}
              </span>
            </div>
            <div style={{ fontSize: 11, color: "#066547", fontWeight: 600, marginTop: 2 }}>
              {hospitalName ??
                (hospitalReady ? "Assigned" : "Searching for a receiving hospital…")}
            </div>
          </div>
        </div>
        <div className="flex items-center gap-3 mt-2 pl-1">
          {["Accepted", "ETA Shared", "Resources"].map((t) => (
      ...
      {/* Map - smallest */}
      <MapView variant="tohospital" height={92} />

      {/* Primary CTA / success state */}
      {!notified ? (
        <button
          onClick={onNotifyHospital}
          disabled={busy || !severityChosen || !hospitalReady}
          className="mt-3 w-full rounded-2xl flex items-center justify-center gap-2 active:scale-[0.99] transition-transform disabled:opacity-60 disabled:cursor-not-allowed"
          style={{
        ...
        >
          ...
          <div className="flex items-center gap-3 pl-1">
            {["Severity Shared", "ETA Shared"].map((t) => (
              <div key={t} className="flex items-center gap-1">
